...
- **Fake scenarios implemented:**
  - All 16 scripted offline scenarios implemented in `Shoestring.Harness.Fake.Scenario`: `normal_completion`, `approaching_reserve`, `sudden_quota_refusal`, `stale_capacity`, `missing_capacity`, `malformed_event`, `start_failure`, `mid_run_crash`, `cancel_before_effect`, `cancel_after_effect`, `lease_expiry_at_safe_boundary`, `same_session_resume`, `handoff_target`, `delayed_delivery`, `duplicated_delivery`, `out_of_order_delivery`.
- **Contract-suite results:**
  - `Shoestring.Harness.ContractSuite` shared test suite implemented in `test/support/harness_contract_suite.ex` and exercised in `test/shoestring/harness/contract_suite_test.exs` (both conforming capability adapter validation and intentional rejection tests for non-conforming adapters).
