...
- `handoff-production.md`: Iteration-5 production cross-provider handoff — the `run.handoff` durable Cobbler command, `Shoestring.Cobbler.Handoffs` (boundary check, one-active-Elf guard, fresh receiver observation + admission, receiver-owned lease grant, `handoff.created`, durable supervised dispatch, idempotent convergence), the `Shoestring.Elves` session-lookup twin of R4.2, measured baseline, and the fail-on-base ledger with its honest lock-vs-documentation split.
